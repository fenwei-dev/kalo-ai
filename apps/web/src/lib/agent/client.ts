...
function ensureConfig() {
	if (!app.aiConfig)
		throw new Error("还没有配置 AI，请先去「设置」填写 API Key 和模型。");
...
function dbToAgentMessage(message: DBMessage): Message {
	...
	if (message.role === "user") {
		const sentAt =
			message.localTimestamp ?? localMessageTimestamp(new Date(timestamp));
...
async function persistToolResult(
	...
	if (
		!message.isError &&
		(message.toolName === "readUserMemory" ||
			message.toolName === "updateUserMemory")
	) {
