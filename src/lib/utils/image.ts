...
export type ImagePreparationErrorCode = 'unsupported' | 'too-large' | 'decode' | 'encode';
...
export async function prepareImage(file: File): Promise<PreparedImage> {
	...
	if (file.size > MAX_IMAGE_SOURCE_BYTES) throw new ImagePreparationError('too-large');

	const decoded = await decodeImage(file);
	try {
		if (!decoded.width || !decoded.height) throw new ImagePreparationError('decode');
		...
		for (let sizeAttempt = 0; sizeAttempt < 7; sizeAttempt++) {
			...
			for (const quality of qualities) {
				for (const outputType of ['image/webp', 'image/jpeg'] as const) {
					const candidate = await canvasBlob(decoded.source, width, height, outputType, quality);
			...
			if (smallest && Math.ceil(smallest.size / 3) * 4 <= MAX_IMAGE_ENCODED_BYTES) {
				return {
