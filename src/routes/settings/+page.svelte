<script lang="ts">
	...
	import { exportAll, clearAllData } from '$lib/db/repositories';
	import { getUser, saveUser } from '$lib/db/repositories';
	...
	async function handleImport(e: Event) {
		...
		try {
			...
			if (data.user?.[0]) {
				// 恢复 user（去掉内部时间戳，重新保存以补齐 createdAt/updatedAt）
		...
		{#if confirmingClear}
			<div class="mt-3 rounded-xl bg-red-50 p-3 text-center">
				...
				<div class="grid grid-cols-2 gap-3">
					...
					<button
						onclick={handleClear}
						class="rounded-full bg-red-500 py-2 text-sm font-medium text-white"
					>确认清空</button
					>
				</div>
			</div>
		{:else}
			<Button clear class="mt-2 !text-red-500" onclick={() => (confirmingClear = true)}>清空所有数据</Button>
		{/if}
	</Block>
